...
function send_booking_email($booking, $template_type) {
    ...
    $bodies = [
        ...
        'informacion_llegada' => [
            'es' => '<h2>Información útil para tu llegada</h2><p>Hola, <strong>{{GUEST_NAME}}</strong>:</p><p>¡Tu viaje a Can Picornell está muy cerca! Te enviamos los datos de acceso para tu llegada:</p><ul><li><strong>Hora de Entrada (Check-in):</strong> A partir de las 16:00.</li><li><strong>Hora de Salida (Check-out):</strong> Antes de las 10:00.</li><li><strong>Guía de Check-In Autónomo:</strong> Puedes consultar el código de la barrera y la guía visual paso a paso en <a href="{{BASE_URL}}es/checkin-autonomo.html" target="_blank">Instrucciones de Check-In Autónomo</a>.</li><li><strong>EcoTasa:</strong> Recuerda tener preparado el importe de la EcoTasa en efectivo ({{TAX_TOTAL}}€) para abonar a la llegada.</li></ul><p>Buen viaje y no dudes en escribirnos si necesitas recomendaciones o tienes alguna pregunta.</p>',
            'en' => '<h2>Useful Arrival Information</h2><p>Hello, <strong>{{GUEST_NAME}}</strong>:</p><p>Your trip to Can Picornell is just around the corner! Here is your check-in and access information:</p><ul><li><strong>Check-in Time:</strong> From 16:00 (4:00 PM).</li><li><strong>Check-out Time:</strong> Before 10:00 AM.</li><li><strong>Self Check-In Guide:</strong> You can check the gate code and step-by-step visual instructions at <a href="{{BASE_URL}}en/checkin-autonomo.html" target="_blank">Self Check-In Instructions</a>.</li><li><strong>EcoTasa:</strong> Please prepare the EcoTasa amount (€{{TAX_TOTAL}}) in cash for payment upon arrival.</li></ul><p>Safe travels! Contact us if you have any questions or need recommendations.</p>',
            'de' => '<h2>Nützliche Anreiseinformationen</h2><p>Hallo, <strong>{{GUEST_NAME}}</strong>:</p><p>Ihre Reise nach Can Picornell steht kurz bevor! Hier sind Ihre Zugangsdaten für die Anreise:</p><ul><li><strong>Check-in Zeit:</strong> Ab 16:00 Uhr.</li><li><strong>Check-out Zeit:</strong> Bis 10:00 Uhr.</li><li><strong>Anleitung zum Selbst-Check-in:</strong> Den Tor-Code und die visuelle Schritt-für-Schritt-Anleitung finden Sie unter <a href="{{BASE_URL}}de/checkin-autonomo.html" target="_blank">Anleitung zum kontaktlosen Check-in</a>.</li><li><strong>EcoTasa:</strong> Bitte halten Sie den EcoTasa-Betrag ({{TAX_TOTAL}} €) bei Ihrer Ankunft in bar bereit.</li></ul><p>Gute Anreise! Zögern Sie nicht, uns bei Fragen oder für Empfehlungen zu kontaktieren.</p>'
        ],
        'cancelacion' => [
